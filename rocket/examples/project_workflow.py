...
def plot_design_comparison(designs: list[dict], figsize: tuple = (14, 10)) -> plt.Figure:
    ...
    ax1.bar(x + width/2, isp_sl, width, label='Sea Level', color=colors[1], edgecolor='white', linewidth=0.5)
    ...
    for i, (name, data) in enumerate(zip(names, metrics, strict=True)):
        values = data.tolist()
        values += values[:1]
...
def plot_trade_study(df: pl.DataFrame, x_col: str, y_cols: list[str],
    ...
    for i, (ax, y_col) in enumerate(zip(axes, y_cols, strict=True)):
...
"""
    ...
    ax_perf.barh(x - width/2, isp_vac, width, label='Vacuum Isp',
                 color='#e94560', edgecolor='white')
    ax_perf.barh(x + width/2, isp_sl, width, label='Sea Level Isp',
